...
export default ({winner, listID, submissionIndex, submitter, deadline, appealFee, amountContributed, rewardPercentage, governorContractInstance, account }) => {
  const amountRemaining = Web3.utils.fromWei((Number(appealFee || 0) - Number(amountContributed || 0)).toString())
  const onFund = (amount) => {
  ...
  return (
    <StyledAppealSideBox>
      ...
      <Progress percent={(Number(amountRemaining)/Number(appealFee)*100).toFixed(0)} style={{marginTop: "25px"}}/>
      ...
      <Input.Search
        ...
        enterButton="Fund"
        size="large"
        disabled={(Number(amountRemaining)/Number(appealFee)*100).toFixed(0) === "100"}
