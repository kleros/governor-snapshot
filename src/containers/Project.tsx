...
const ProjectHome: React.FC<{ match: ProjectParams }> = (props) => {
  ...
  const [cachedPendingLists, setPendingListsCache] = useLocalStorage(
    "CACHED_LISTS",
    undefined
  );
  ...
  const governorContractInstance = useMemo(
    () =>
      new web3.eth.Contract(governorInterfaceABI, projectInfo.governorAddress),
    []
  );

  const arbitratorContractInstance = useMemo(
    () =>
  ...
  return (
    <StyledProjectHome>
          ...
          <StyledHeader>
            <Link to="/">Governor</Link>
            {
              <span style={{ fontWeight: "300" }}>
                ...
                / {projectInfo.icon}
                {capitalizeString(
                  `${params.projectName} @ ${projectInfo.chain.name}`
                )}
              </span>
            }
          </StyledHeader>
      ...
      {correctChain ? (
        ...
        <div>
          <StyledInvalidNetwork>Invalid network.</StyledInvalidNetwork>
        </div>
      )}
    </StyledProjectHome>
  );
